...
plt.plot(xGen, y, label="Megabytes(t) = t^2e^t")
plt.plot(xGen, y_derivada, label="Derivada: y = e^t(t^2 + 2t)")
...
plt.scatter(
    x1, fx, color="Red", s=30, zorder=5, label=f"x1 = ({round(x1,2)}, {round(fx, 2)})"
)  # punto evaluado en la funcion
